...
OSStatus io_proc(
  ...
  if (io_proc_data->frame % 30 == 0) {
    float frequency = io_proc_data->note_table[
      io_proc_data->scale[
        io_proc_data->frame % io_proc_data->length_scale
      ] + (
        (io_proc_data->frame / io_proc_data->steps_notes) % (io_proc_data->octave_range / 2)
  ...
  if (
    io_proc_data->frame % 20 == 0
  ) {
    float frequency = io_proc_data->note_table[
      io_proc_data->scale[
        io_proc_data->frame % io_proc_data->length_scale
      ] + (
        (io_proc_data->frame / io_proc_data->steps_notes) % io_proc_data->octave_range
      ) * io_proc_data->steps_notes
    ];
